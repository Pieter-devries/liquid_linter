...
function runCustomChecks(code, parameter) {
    ...
    while ((match = variableRegex.exec(code)) !== null) {
        ...
        if (varName.startsWith('_') && !allowedVariables.includes(varName)) {
            let suggestion = '';
            if (varName === '_isfiltered')
                suggestion = 'Did you mean `_is_filtered`?';
            else if (varName === '_inquery')
                suggestion = 'Did you mean `_in_query`?';
            else if (varName === '_renderedvalue')
                suggestion = 'Did you mean `_rendered_value`?';
            else if (varName === '_linkedvalue')
                suggestion = 'Did you mean `_linked_value`?';
            errors.push({
                type: 'Looker-specific',
...
function validateParameterUsage(code, parameter, errors) {
    ...
    while ((match = variableRegex.exec(code)) !== null) {
        ...
        if (varName.startsWith('_') && !supported.includes(varName)) {
            if (varName === '_filters' || varName === '_user_attributes' || varName === '_localization')
                continue;
